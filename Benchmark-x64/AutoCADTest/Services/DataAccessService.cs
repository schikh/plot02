...
namespace AutoCADTest.Service
{
    public class DataAccessService
    {
        ...
        public IEnumerable<T> IterateOverReader<T>(string query, Func<IDataReader, T> mapReader, int top=0)
        {
            var list = new List<T>();
            if (top > 0)
            {
                query = string.Format("SELECT * FROM ({0}) WHERE ROWNUM <= {1}", query, top);
            }
            using (var connection = new OracleConnection(_connectionString))
            using (var command = new OracleCommand(query, connection))
            {
