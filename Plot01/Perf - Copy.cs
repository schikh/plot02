...
namespace BatchPlot
{
    public class Perf : IExtensionApplication
    {
        ...
        private int _plotCartridgeWidth = 200;
        ...
        private void SetPlotSettings(Layout layout, PageSettings pageSettings)
        {
            using (var ps = new PlotSettings(layout.ModelType))
            {
                ...
                var height = _drawingSize.Height * _planchetteScale + 2 * _internalBorderWidth + 2 * _externalBorderWidth;
                var width = _drawingSize.Width * _planchetteScale + 2 * _internalBorderWidth + 2 * _externalBorderWidth 
                    + _plotCartridgeWidth + 2 * _cartridgeExternalBorderWidth;
        ...
        private IEnumerable<PageFormat> GetPageFormatList(PlotSettingsValidator psv, PlotSettings ps)
        {
            ...
            for (int i = 0; i < medlist.Count; i++)
            {
                ...
                {
                    //LocaleMediaName = psv.GetLocaleMediaName(ps, i),
                    CanonicalMediaName = ps.CanonicalMediaName,
                    PlotPaperSize = new Point2d(ps.PlotPaperSize.X - ps.PlotPaperMargins.MinPoint.X - ps.PlotPaperMargins.MaxPoint.X,
                        ps.PlotPaperSize.Y - ps.PlotPaperMargins.MinPoint.Y - ps.PlotPaperMargins.MaxPoint.Y),
                    //PlotRotation = ps.PlotRotation
                };
            }
        }
